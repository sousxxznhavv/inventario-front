...
function TipoDocumentoForm() {
  ...
  const guardarTipoDocumento = async () => {
    try {
      const response = await fetch("http://localhost:3000/api/tipoDocumentos", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ descrp: tipoDocumento }),
      });
      if (!response.ok) {
      ...
      >
        <Box sx={{ textAlign: "center", padding: "10px" }}>
          ...
          <Box sx={{ textAlign: "right", padding: "0px" }}>
            <Button variant="contained" onClick={guardarTipoDocumento}>
              GUARDAR
            </Button>
          </Box>
        </Box>
